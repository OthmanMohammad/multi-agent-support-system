...
from src.database.models import ABTest, ConversationAnalytics, FeatureUsage
...
class ConversationAnalyticsRepository(BaseRepository[ConversationAnalytics]):
    ...
    async def get_by_conversation(self, conversation_id: UUID) -> ConversationAnalytics | None:
    ...
    async def get_average_resolution_time(
        ...
        result = await self.session.execute(
            select(func.avg(ConversationAnalytics.resolution_time_seconds)).where(and_(*conditions))
        )
    ...
    async def get_escalation_rate(
        ...
        if conditions:
            total_result = await self.session.execute(
                select(func.count(ConversationAnalytics.id)).where(and_(*conditions))
            )
            escalated_result = await self.session.execute(
                select(func.count(ConversationAnalytics.id)).where(
                    and_(ConversationAnalytics.was_escalated, *conditions)
                )
            )
        else:
            total_result = await self.session.execute(select(func.count(ConversationAnalytics.id)))
            escalated_result = await self.session.execute(
                select(func.count(ConversationAnalytics.id)).where(
                    ConversationAnalytics.was_escalated
                )
            )
...
class FeatureUsageRepository(BaseRepository[FeatureUsage]):
    ...
    async def get_by_customer(self, customer_id: UUID, limit: int = 100) -> list[FeatureUsage]:
    ...
    async def get_by_feature(self, feature_name: str, limit: int = 100) -> list[FeatureUsage]:
    ...
    async def get_feature_adoption_rate(self, feature_name: str) -> float:
        ...
        feature_result = await self.session.execute(
            select(func.count(func.distinct(FeatureUsage.customer_id))).where(
...
class ABTestRepository(BaseRepository[ABTest]):
    ...
    async def get_by_customer(self, customer_id: UUID) -> list[ABTest]:
    ...
    async def get_by_test_name(self, test_name: str, limit: int = 1000) -> list[ABTest]:
        """Get all participants in a specific test"""
        result = await self.session.execute(
            select(ABTest).where(ABTest.test_name == test_name).limit(limit)
        )
        return list(result.scalars().all())

    async def get_by_variant(self, test_name: str, variant: str, limit: int = 1000) -> list[ABTest]:
        """Get all participants in a specific variant"""
        result = await self.session.execute(
            select(ABTest)
            .where(and_(ABTest.test_name == test_name, ABTest.variant == variant))
            .limit(limit)
        )
        return list(result.scalars().all())
    ...
    async def get_test_results(self, test_name: str) -> dict:
        ...
        for row in result:
            conversion_rate = (
                (row.conversions / row.participants * 100) if row.participants > 0 else 0.0
            )
            results[row.variant] = {
    ...
    async def get_active_tests(self) -> list[str]:
        ...
        result = await self.session.execute(select(func.distinct(ABTest.test_name)))
