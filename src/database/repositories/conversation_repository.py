...
class ConversationRepository(BaseRepository[Conversation]):
    ...
    async def create_with_customer(self, customer_id: UUID, **kwargs) -> Conversation:
        """
    ...
    async def get_by_customer(
        self, customer_id: UUID, limit: int = 50, status: str | None = None
    ) -> list[Conversation]:
        """
    ...
    async def get_with_customer(self, conversation_id: UUID) -> Conversation | None:
        """
    ...
    async def mark_resolved(
        self, conversation_id: UUID, resolution_time_seconds: int
    ) -> Conversation | None:
        """
    ...
    async def mark_escalated(self, conversation_id: UUID) -> Conversation | None:
        """
        ...
        Returns:
            Updated conversation or None
        """
        return await self.update(conversation_id, status="escalated", ended_at=datetime.now(UTC))

    async def search_by_intent(self, intent: str, limit: int = 50) -> list[Conversation]:
        """
    ...
    async def get_by_date_range(
        self, start_date: datetime, end_date: datetime, customer_id: UUID | None = None
    ) -> list[Conversation]:
        """
        ...
        query = select(Conversation).where(
            and_(Conversation.started_at >= start_date, Conversation.started_at <= end_date)
        )
    ...
    async def get_unresolved_conversations(self, hours_old: int = 24) -> list[Conversation]:
        """
        ...
        result = await self.session.execute(
            select(Conversation)
            .where(and_(Conversation.status == "active", Conversation.started_at < cutoff))
    ...
    async def get_statistics(self, days: int = 7) -> dict:
        ...
        intent_result = await self.session.execute(
            ...
            .where(and_(Conversation.started_at >= since, Conversation.primary_intent.isnot(None)))
        ...
        avg_sentiment_result = await self.session.execute(
            select(func.avg(Conversation.sentiment_avg)).where(
                and_(Conversation.started_at >= since, Conversation.sentiment_avg.isnot(None))
            )
        )
    ...
    async def get_agent_usage_stats(self, days: int = 7) -> dict:
        ...
        result = await self.session.execute(
            select(Conversation).where(
                and_(Conversation.started_at >= since, Conversation.agents_involved.isnot(None))
            )
        )
