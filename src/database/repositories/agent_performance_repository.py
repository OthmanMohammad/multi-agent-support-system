...
class AgentPerformanceRepository(BaseRepository[AgentPerformance]):
    ...
    async def record_daily_metrics(
        self, agent_name: str, date: datetime, **metrics
    ) -> AgentPerformance:
        """
        ...
        result = await self.session.execute(
            select(AgentPerformance).where(
                and_(AgentPerformance.agent_name == agent_name, AgentPerformance.date == date)
            )
        )
        ...
        if existing:
            ...
            # Create new
            return await self.create(agent_name=agent_name, date=date, extra_metadata={}, **metrics)

    async def get_agent_metrics(self, agent_name: str, days: int = 30) -> list[AgentPerformance]:
        """
        ...
        result = await self.session.execute(
            ...
            .where(and_(AgentPerformance.agent_name == agent_name, AgentPerformance.date >= since))
    ...
    async def get_performance_trend(self, agent_name: str, days: int = 30) -> list[dict]:
        """
    ...
    async def get_top_performers(self, days: int = 7, limit: int = 10) -> list[AgentPerformance]:
        """
    ...
    async def compare_agents(self, agent_names: list[str], days: int = 7) -> dict:
        """
        ...
        for agent_name in agent_names:
            result = await self.session.execute(
                select(
                    ...
                    and_(AgentPerformance.agent_name == agent_name, AgentPerformance.date >= since)
                )
            )
