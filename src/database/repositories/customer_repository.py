...
class CustomerRepository(BaseRepository[Customer]):
    ...
    async def get_or_create_by_email(
        ...
        if not customer:
            customer = await self.create(email=email, name=name, plan=plan, extra_metadata={})
    ...
    async def upgrade_plan(self, customer_id: UUID, new_plan: str) -> Customer | None:
        """
    ...
    async def get_plan_distribution(self) -> dict:
        ...
        """
        result = await self.session.execute(
            select(Customer.plan, func.count(Customer.id).label("count")).group_by(Customer.plan)
        )
