...
class MessageRepository(BaseRepository[Message]):
    ...
    async def get_latest_message(self, conversation_id: UUID) -> Message | None:
        """
    ...
    async def get_by_agent(self, agent_name: str, limit: int = 100) -> list[Message]:
        """
    ...
    async def get_by_role(self, conversation_id: UUID, role: str) -> list[Message]:
        """
        ...
        result = await self.session.execute(
            select(Message)
            .where(and_(Message.conversation_id == conversation_id, Message.role == role))
            .order_by(Message.created_at.asc())
        )
        return list(result.scalars().all())

    async def get_sentiment_distribution(self, conversation_id: UUID) -> dict:
        """
        ...
        result = await self.session.execute(
            select(
                ...
                func.count(
                    case((and_(Message.sentiment >= -0.3, Message.sentiment <= 0.3), 1))
                ).label("neutral"),
                func.avg(Message.sentiment).label("average"),
            ).where(and_(Message.conversation_id == conversation_id, Message.sentiment.isnot(None)))
        )
    ...
    async def get_average_confidence(self, conversation_id: UUID) -> float:
        """
        ...
        result = await self.session.execute(
            select(func.avg(Message.confidence)).where(
                and_(Message.conversation_id == conversation_id, Message.confidence.isnot(None))
            )
        )
        avg = result.scalar()
        return round(float(avg), 2) if avg else 0.0

    async def get_token_usage(self, conversation_id: UUID | None = None, days: int = 7) -> dict:
        """
        ...
        ).where(and_(Message.created_at >= since, Message.tokens_used.isnot(None)))
    ...
    async def search_content(self, search_term: str, limit: int = 50) -> list[Message]:
        """
