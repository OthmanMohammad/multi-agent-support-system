...
class CustomerHealthEventRepository(BaseRepository[CustomerHealthEvent]):
    ...
    async def get_by_severity(self, customer_id: UUID, severity: str) -> list[CustomerHealthEvent]:
...
class CustomerSegmentRepository(BaseRepository[CustomerSegment]):
    ...
    async def get_by_customer(self, customer_id: UUID) -> list[CustomerSegment]:
    ...
    async def get_segment_distribution(self) -> dict:
        ...
        result = await self.session.execute(
            select(
                CustomerSegment.segment_name, func.count(CustomerSegment.id).label("count")
...
class CustomerNoteRepository(BaseRepository[CustomerNote]):
    ...
    async def get_by_customer(
        self, customer_id: UUID, note_type: str | None = None, limit: int = 100
    ) -> list[CustomerNote]:
    ...
    async def get_by_author(self, author_id: UUID, limit: int = 100) -> list[CustomerNote]:
        """Get notes by author"""
        result = await self.session.execute(
            select(CustomerNote)
...
class CustomerContactRepository(BaseRepository[CustomerContact]):
    ...
    async def get_by_email(self, email: str) -> CustomerContact | None:
    ...
    async def get_primary_contact(self, customer_id: UUID) -> CustomerContact | None:
...
class CustomerIntegrationRepository(BaseRepository[CustomerIntegration]):
    ...
    async def get_by_customer(self, customer_id: UUID) -> list[CustomerIntegration]:
    ...
    async def get_by_type(
        self, customer_id: UUID, integration_type: str
    ) -> CustomerIntegration | None:
        """Get specific integration type for a customer"""
        result = await self.session.execute(
            select(CustomerIntegration).where(
                and_(
