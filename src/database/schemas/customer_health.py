...
class CustomerHealthEventBase(BaseModel):
    ...
    customer_id: UUID
    event_type: str = Field(..., pattern="^(health_score_change|churn_risk_change|engagement_drop|usage_spike)$")
...
class CustomerHealthEventUpdate(BaseModel):
    ...
    event_type: Optional[str] = Field(None, pattern="^(health_score_change|churn_risk_change|engagement_drop|usage_spike)$")
    old_value: Optional[Decimal] = None
    new_value: Optional[Decimal] = None
    reason: Optional[str] = None
    detected_by: Optional[str] = None
...
class CustomerSegmentUpdate(BaseModel):
    ...
    segment_type: Optional[str] = Field(None, pattern="^(industry|lifecycle|value|behavior|risk)$")
    confidence_score: Optional[Decimal] = None
    assigned_by: Optional[str] = Field(None, max_length=50)
    extra_metadata: Optional[dict] = None
...
class CustomerNoteUpdate(BaseModel):
    ...
    note_type: Optional[str] = Field(None, pattern="^(general|support|sales|success|technical)$")
    content: Optional[str] = None
    is_internal: Optional[bool] = None
    visibility: Optional[str] = Field(None, pattern="^(private|team|company)$")
    extra_metadata: Optional[dict] = None
...
class CustomerContactBase(BaseModel):
    ...
    role: Optional[str] = Field(None, pattern="^(admin|billing|technical|business|user)$")
...
class CustomerIntegrationBase(BaseModel):
    ...
    last_sync_status: Optional[str] = Field(None, max_length=20)
    error_count: int = 0
    extra_metadata: dict = Field(default_factory=dict)
...
class CustomerIntegrationUpdate(BaseModel):
    ...
    integration_type: Optional[str] = Field(None, max_length=50)
    status: Optional[str] = Field(None, pattern="^(active|paused|error|disconnected)$")
