...
class Conversation(BaseModel):
    ...
    # Timestamps
    started_at = Column(DateTime(timezone=True), server_default="now()", nullable=False, index=True)
    ...
    handoffs = relationship(
        "AgentHandoff", back_populates="conversation", cascade="all, delete-orphan", lazy="selectin"
    )
    ...
    ab_tests = relationship(
        "ABTest", back_populates="conversation", cascade="all, delete-orphan", lazy="selectin"
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('active', 'resolved', 'escalated')", name="check_conversation_status_values"
        ),
        CheckConstraint(
