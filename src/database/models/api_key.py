...
class APIKey(BaseModel):
    ...
    name: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="Human-readable key name"
    )
    ...
    key_hash: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="Bcrypt hash of the API key"
    )
    ...
    last_used_ip: Mapped[str | None] = mapped_column(
        String(50), nullable=True, comment="IP address of last use"
    )

    usage_count: Mapped[int] = mapped_column(
        nullable=False, default=0, comment="Number of times key has been used"
    )

    # Status
    is_active: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True, index=True, comment="Whether key is active"
    )

    # Metadata
    description: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="Optional description of key purpose"
    )
