...
class CustomerApplicationService:
    ...
    async def create_customer(
        ...
        """Create new customer"""
        try:
            self.logger.info("customer_creation_started", email=email, plan=plan)

            # Validate email
            validation = self.domain.validate_email(email)
            if validation.is_failure:
                self.logger.warning(
                    "customer_creation_validation_failed", email=email, error="invalid_email"
                )
            ...
            self.logger.info(
                "customer_created", customer_id=str(customer.id), email=email, plan=plan
            )
            ...
            return Result.fail(
                InternalError(
                    message=f"Failed to create customer: {e!s}",
                    operation="create_customer",
                    component="CustomerApplicationService",
                )
            )

    async def upgrade_plan(self, customer_id: UUID, new_plan: str) -> Result[dict[str, Any]]:
        ...
        try:
            ...
            return Result.fail(
                InternalError(
                    message=f"Failed to upgrade plan: {e!s}",
                    operation="upgrade_plan",
                    component="CustomerApplicationService",
                )
            )

    async def downgrade_plan(self, customer_id: UUID, new_plan: str) -> Result[dict[str, Any]]:
        ...
        try:
            ...
            return Result.fail(
                InternalError(
                    message=f"Failed to downgrade plan: {e!s}",
                    operation="downgrade_plan",
                    component="CustomerApplicationService",
                )
            )

    async def get_customer_profile(self, customer_id: UUID) -> Result[dict[str, Any]]:
        """Get customer profile with statistics"""
        try:
            self.logger.debug("customer_profile_requested", customer_id=str(customer_id))

            result = await self.infrastructure.get_customer_profile(customer_id)

            if result.is_success:
                self.logger.info("customer_profile_retrieved", customer_id=str(customer_id))
    ...
    async def list_customers(self, plan: str | None = None, limit: int = 50) -> Result[list]:
        """List customers with optional filtering"""
        try:
            self.logger.debug("list_customers_started", plan=plan, limit=limit)
            ...
            self.logger.info("customers_listed", count=len(result), plan=plan)

            return Result.ok(result)

        except Exception as e:
            self.logger.error("list_customers_failed", error=str(e), exc_info=True)
            return Result.fail(
                InternalError(
                    message=f"Failed to list customers: {e!s}",
                    operation="list_customers",
                    component="CustomerApplicationService",
                )
            )

    async def delete_customer(self, customer_id: UUID) -> Result[None]:
        """Delete a customer"""
        try:
            self.logger.info("delete_customer_started", customer_id=str(customer_id))
            ...
            self.logger.info("customer_deleted", customer_id=str(customer_id))

            return Result.ok(None)

        except Exception as e:
            self.logger.error(
                "delete_customer_failed", customer_id=str(customer_id), error=str(e), exc_info=True
            )
            return Result.fail(
                InternalError(
                    message=f"Failed to delete customer: {e!s}",
                    operation="delete_customer",
                    component="CustomerApplicationService",
                )
            )
