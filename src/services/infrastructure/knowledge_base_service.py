...
class KnowledgeBaseService:
    ...
    async def search_articles(
        ...
        try:
            ...
            return Result.fail(
                ExternalServiceError(
                    message=f"Failed to search knowledge base: {e!s}",
                    service="Qdrant",
                    operation="search",
                    is_retryable=True,
                )
    ...
    async def get_article_by_id(self, article_id: str) -> Result[dict | None]:
        ...
        return Result.fail(NotFoundError(resource="Article", identifier=article_id))

    async def track_article_usage(
        self, conversation_id: UUID, article_ids: list[str]
    ) -> Result[None]:
    ...
    async def get_popular_articles(self, days: int = 30, limit: int = 10) -> Result[list[dict]]:
        """Get most-used articles"""
        self.logger.debug("kb_popular_articles_not_implemented", days=days, limit=limit)
        return Result.ok([])

    async def search_by_category(self, category: str, limit: int = 10) -> Result[list[dict]]:
        ...
        try:
            self.logger.info("kb_category_search_started", category=category, limit=limit)

            results = self.vector_store.search(
                query="", category=category, limit=limit, score_threshold=0.0
            )

            self.logger.info(
                "kb_category_search_completed", category=category, results_count=len(results)
            )
            return Result.ok(results)

        except Exception as e:
            self.logger.error(
                "kb_category_search_failed", category=category, error=str(e), exc_info=True
            )
            return Result.fail(
                ExternalServiceError(
                    message=f"Failed to get articles by category: {e!s}",
                    service="Qdrant",
                    operation="search_by_category",
                    is_retryable=True,
                )
    ...
    async def get_collection_info(self) -> Result[dict]:
        ...
        try:
            ...
            self.logger.info(
                "kb_collection_info_retrieved", points_count=info.get("points_count", 0)
            )
            return Result.ok(info)

        except Exception as e:
            self.logger.error("kb_collection_info_failed", error=str(e), exc_info=True)
            return Result.fail(
                ExternalServiceError(
                    message=f"Failed to get collection info: {e!s}",
                    service="Qdrant",
                    operation="get_collection_info",
                    is_retryable=False,
                )
    ...
    async def add_article(
        self, title: str, content: str, category: str, tags: list[str]
    ) -> Result[str]:
        """Add new article to KB"""
        self.logger.debug("kb_add_article_not_implemented", title=title, category=category)
        return Result.fail(
            ExternalServiceError(
    ...
    async def update_article(self, article_id: str, content: str) -> Result[None]:
    ...
    async def delete_article(self, article_id: str) -> Result[None]:
