...
class AnalyticsService:
    ...
    async def get_conversation_statistics(self, days: int = 7) -> Result[dict]:
        """
        ...
        """
        try:
            self.logger.debug("conversation_statistics_requested", days=days)
            ...
            return Result.fail(
                InternalError(
                    message=f"Failed to get conversation statistics: {e!s}",
                    operation="get_conversation_statistics",
                    component="AnalyticsService",
                )
            )

    async def get_agent_performance(self, agent_name: str, days: int = 7) -> Result[dict]:
        """
        ...
        """
        try:
            self.logger.debug("agent_performance_requested", agent_name=agent_name, days=days)
            ...
            if not metrics_list:
                self.logger.info("agent_performance_no_data", agent_name=agent_name, days=days)
                return Result.ok(
                    {
            ...
            weighted_confidence = sum(
                (m.avg_confidence or 0) * m.total_interactions for m in metrics_list
            )
            weighted_sentiment = sum(
                (m.avg_sentiment or 0) * m.total_interactions for m in metrics_list
            )

            avg_confidence = (
                weighted_confidence / total_interactions if total_interactions > 0 else 0.0
            )
            avg_sentiment = (
                weighted_sentiment / total_interactions if total_interactions > 0 else 0.0
            )
            success_rate = (
                (successful_resolutions / total_interactions * 100)
            ...
            self.logger.error(
                "agent_performance_failed", agent_name=agent_name, error=str(e), exc_info=True
            )
            return Result.fail(
                InternalError(
                    message=f"Failed to get agent performance: {e!s}",
                    operation="get_agent_performance",
                    component="AnalyticsService",
                )
            )

    async def get_customer_satisfaction_scores(self, days: int = 30) -> Result[dict]:
        """Get customer satisfaction metrics based on sentiment"""
        try:
            self.logger.debug("csat_scores_requested", days=days)

            since = datetime.now(UTC) - timedelta(days=days)

            conversations = await self.uow.conversations.get_by_date_range(
                start_date=since, end_date=datetime.now(UTC)
            )

            with_sentiment = [c for c in conversations if c.sentiment_avg is not None]
            ...
            self.logger.error("csat_scores_failed", days=days, error=str(e), exc_info=True)
            return Result.fail(
                InternalError(
                    message=f"Failed to get CSAT scores: {e!s}",
                    operation="get_customer_satisfaction_scores",
                    component="AnalyticsService",
                )
            )

    async def get_intent_distribution(self, days: int = 7) -> Result[dict[str, int]]:
        ...
        try:
            ...
            self.logger.info(
                "intent_distribution_retrieved", days=days, unique_intents=len(intent_dist)
            )
            return Result.ok(intent_dist)

        except Exception as e:
            self.logger.error("intent_distribution_failed", days=days, error=str(e), exc_info=True)
            return Result.fail(
                InternalError(
                    message=f"Failed to get intent distribution: {e!s}",
                    operation="get_intent_distribution",
                    component="AnalyticsService",
                )
            )

    async def get_resolution_time_trends(self, days: int = 7) -> Result[dict]:
        ...
        try:
            ...
            self.logger.error(
                "resolution_time_trends_failed", days=days, error=str(e), exc_info=True
            )
            return Result.fail(
                InternalError(
                    message=f"Failed to get resolution time trends: {e!s}",
                    operation="get_resolution_time_trends",
                    component="AnalyticsService",
                )
            )

    async def get_escalation_rate(self, days: int = 7) -> Result[dict]:
        """Get escalation rate"""
        try:
            stats = await self.uow.conversations.get_statistics(days=days)
            ...
            self.logger.error("escalation_rate_failed", days=days, error=str(e), exc_info=True)
            return Result.fail(
                InternalError(
                    message=f"Failed to get escalation rate: {e!s}",
                    operation="get_escalation_rate",
                    component="AnalyticsService",
                )
            )

    async def get_kb_effectiveness(self, days: int = 7) -> Result[dict]:
        ...
        try:
            ...
            with_kb = [
                c for c in conversations if c.kb_articles_used and len(c.kb_articles_used) > 0
            ]
            ...
            self.logger.error("kb_effectiveness_failed", days=days, error=str(e), exc_info=True)
            return Result.fail(
                InternalError(
                    message=f"Failed to get KB effectiveness: {e!s}",
    ...
    async def track_agent_interaction(
        self, agent_name: str, success: bool, confidence: float
    ) -> Result[None]:
        ...
        try:
            ...
            self.logger.warning(
                "agent_interaction_tracking_failed", agent_name=agent_name, error=str(e)
            )
            return Result.ok(None)

    async def get_agent_comparison(self, days: int = 7) -> Result[dict]:
        """Compare performance across all agents"""
        try:
            summary = await self.uow.agent_performance.get_all_agents_summary(days=days)

            self.logger.info("agent_comparison_retrieved", days=days, agent_count=len(summary))
            return Result.ok(summary)

        except Exception as e:
            self.logger.error("agent_comparison_failed", days=days, error=str(e), exc_info=True)
            return Result.fail(
                InternalError(
                    message=f"Failed to get agent comparison: {e!s}",
