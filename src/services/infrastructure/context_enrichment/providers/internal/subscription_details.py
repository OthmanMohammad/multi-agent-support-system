...
class SubscriptionDetailsProvider(BaseContextProvider):
    ...
    async def _fetch_with_session(self, customer_id: UUID, session: AsyncSession) -> dict[str, Any]:
        ...
        try:
            ...
            overdue_count = sum(
                1 for inv in invoices if hasattr(inv, "status") and inv.status == "overdue"
            )
            ...
            for payment in payments:
                if (
                    ...
                    and hasattr(payment, "created_at")
                    and payment.created_at.replace(tzinfo=None) > cutoff_date
                ):
            ...
            return {
                "subscription": {
                    ...
                    if hasattr(subscription, "billing_cycle")
                    else "monthly",
                    ...
                    if hasattr(subscription, "seats_total")
                    else 1,
                    "seats_used": subscription.seats_used
                    if hasattr(subscription, "seats_used")
                    else 1,
                    "seat_utilization": subscription.seat_utilization
                    if hasattr(subscription, "seat_utilization")
                    else 100.0,
                },
                "billing": {
