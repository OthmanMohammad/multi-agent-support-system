...
from src.database.models import Conversation, Message, AgentPerformance
    ...
    async def _fetch_with_session(
        ...
        try:
            ...
            resolved = sum(
                ...
                if hasattr(conv, 'status') and conv.status == 'resolved'
            )

            escalated = sum(
                1 for conv in conversations
                if hasattr(conv, 'escalated') and conv.escalated
            )
            ...
            for conv in conversations:
                if (hasattr(conv, 'resolved_at') and conv.resolved_at and
                    hasattr(conv, 'started_at') and conv.started_at):
            ...
            # Get CSAT scores
            csat_scores = [
                conv.csat_score for conv in conversations
