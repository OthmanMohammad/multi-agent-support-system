...
from src.core.errors import InternalError, NotFoundError
...
class CustomerInfrastructureService:
    ...
    async def get_or_create_by_email(
        self, email: str, name: str | None = None, plan: str = "free"
    ) -> Result[Customer]:
        """
        ...
        """
        try:
            self.logger.debug("customer_get_or_create_started", email=email, plan=plan)

            customer = await self.uow.customers.get_or_create_by_email(
                email=email, name=name, plan=plan
            )
            ...
            return Result.fail(
                InternalError(
                    message=f"Failed to get/create customer: {e!s}",
    ...
    async def get_by_id(self, customer_id: UUID) -> Result[Customer]:
        ...
        try:
            ...
            if not customer:
                ...
                return Result.fail(NotFoundError(resource="Customer", identifier=str(customer_id)))

            self.logger.debug("customer_retrieved_by_id", customer_id=str(customer_id))
            return Result.ok(customer)
            ...
            return Result.fail(
                InternalError(
                    message=f"Failed to fetch customer: {e!s}",
                    operation="get_by_id",
                    component="CustomerInfrastructureService",
                )
            )

    async def get_by_email(self, email: str) -> Result[Customer | None]:
        """Get customer by email"""
        try:
            customer = await self.uow.customers.get_by_email(email)

            self.logger.debug("customer_search_by_email", email=email, found=customer is not None)
            return Result.ok(customer)

        except Exception as e:
            self.logger.error(
                "customer_get_by_email_failed", email=email, error=str(e), exc_info=True
            )
            return Result.fail(
                InternalError(
                    message=f"Failed to fetch customer by email: {e!s}",
                    operation="get_by_email",
                    component="CustomerInfrastructureService",
                )
            )

    async def upgrade_plan(self, customer_id: UUID, new_plan: str) -> Result[Customer]:
        """Upgrade customer plan"""
        try:
            self.logger.info(
                "customer_plan_upgrade_started", customer_id=str(customer_id), new_plan=new_plan
            )

            updated = await self.uow.customers.update(
                customer_id, plan=new_plan, updated_by=self.uow.current_user_id
            )
            ...
            self.logger.info(
                "customer_plan_upgraded", customer_id=str(customer_id), new_plan=new_plan
            )
            ...
            return Result.fail(
                InternalError(
                    message=f"Failed to upgrade customer plan: {e!s}",
                    operation="upgrade_plan",
                    component="CustomerInfrastructureService",
                )
            )

    async def downgrade_plan(self, customer_id: UUID, new_plan: str) -> Result[Customer]:
        """Downgrade customer plan"""
        try:
            self.logger.info(
                "customer_plan_downgrade_started", customer_id=str(customer_id), new_plan=new_plan
            )
            ...
            self.logger.info(
                "customer_plan_downgraded", customer_id=str(customer_id), new_plan=new_plan
            )
            return Result.ok(updated)
            ...
            return Result.fail(
                InternalError(
                    message=f"Failed to downgrade customer plan: {e!s}",
    ...
    async def get_conversation_count_for_date(
        self, customer_id: UUID, date: datetime
    ) -> Result[int]:
        ...
        try:
            ...
            count = await self.uow.conversations.count(
                customer_id=customer_id, started_at__gte=start_of_day, started_at__lt=end_of_day
            )
            ...
            return Result.fail(
                InternalError(
                    message=f"Failed to count conversations: {e!s}",
                    operation="get_conversation_count_for_date",
                    component="CustomerInfrastructureService",
                )
            )

    async def get_usage_stats(self, customer_id: UUID, days: int = 30) -> Result[dict]:
        """Get customer usage statistics"""
        try:
            since = datetime.now(UTC) - timedelta(days=days)

            conversations = await self.uow.conversations.get_by_customer(
                customer_id=customer_id, limit=1000
            )

            recent_convs = [c for c in conversations if c.started_at >= since]
            ...
            stats = {
                ...
                "avg_sentiment": sum(c.sentiment_avg or 0 for c in recent_convs) / len(recent_convs)
                if recent_convs
                else 0,
                "period_days": days,
            }
            ...
            return Result.fail(
                InternalError(
                    message=f"Failed to get usage stats: {e!s}",
                    operation="get_usage_stats",
                    component="CustomerInfrastructureService",
                )
            )

    async def block_customer(self, customer_id: UUID, reason: str) -> Result[Customer]:
        """Block customer account"""
        try:
            self.logger.warning(
                "customer_block_started", customer_id=str(customer_id), reason=reason
            )
            ...
            updated = await self.uow.customers.update(
                customer_id, extra_metadata=metadata, updated_by=self.uow.current_user_id
            )

            self.logger.warning("customer_blocked", customer_id=str(customer_id), reason=reason)
            return Result.ok(updated)

        except Exception as e:
            self.logger.error(
                "customer_block_failed", customer_id=str(customer_id), error=str(e), exc_info=True
            )
            return Result.fail(
                InternalError(
                    message=f"Failed to block customer: {e!s}",
                    operation="block_customer",
                    component="CustomerInfrastructureService",
                )
            )

    async def unblock_customer(self, customer_id: UUID) -> Result[Customer]:
        """Unblock customer account"""
        try:
            self.logger.info("customer_unblock_started", customer_id=str(customer_id))
            ...
            self.logger.info("customer_unblocked", customer_id=str(customer_id))
            return Result.ok(updated)

        except Exception as e:
            self.logger.error(
                "customer_unblock_failed", customer_id=str(customer_id), error=str(e), exc_info=True
            )
            return Result.fail(
                InternalError(
                    message=f"Failed to unblock customer: {e!s}",
                    operation="unblock_customer",
                    component="CustomerInfrastructureService",
                )
            )

    async def get_customer_profile(self, customer_id: UUID) -> Result[dict]:
        ...
        try:
            ...
            profile = {
                ...
                "blocked": customer.extra_metadata.get("blocked", False)
                if customer.extra_metadata
                else False,
            }

            self.logger.debug("customer_profile_retrieved", customer_id=str(customer_id))
            ...
            return Result.fail(
                InternalError(
                    message=f"Failed to get customer profile: {e!s}",
