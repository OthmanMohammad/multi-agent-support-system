...
class WorkflowStateManager:
    ...
    def create_initial_state(
        ...
        Returns:
            Initial AgentState
        """
        self.logger.debug("creating_initial_state", message_preview=message[:50])

        # Extract context parameters
        conversation_id = context.get("conversation_id") if context else None
        customer_id = (
            context.get("customer_id", "default_customer") if context else "default_customer"
        )
        ...
        try:
            ...
            self.logger.error("initial_state_validation_failed", error=str(e), exc_info=True)
            raise
        ...
        self.logger.debug("initial_state_created", conversation_id=state["conversation_id"])
    ...
    def update_state(self, state: AgentState, updates: dict[str, Any]) -> AgentState:
        ...
        try:
            ...
            self.logger.error("state_validation_failed_after_update", error=str(e), exc_info=True)
            raise
    ...
    def validate_state(self, state: AgentState) -> None:
        ...
        required_fields = ["conversation_id", "customer_id", "current_message", "status"]

        for field in required_fields:
            if field not in state or state[field] is None:
                raise InvalidStateError(f"Missing required field: {field}", invalid_field=field)
    ...
    def validate_state_transition(self, old_state: AgentState, new_state: AgentState) -> None:
        """
        ...
        if old_state["conversation_id"] != new_state["conversation_id"]:
            raise InvalidStateError(
                "conversation_id cannot change during workflow", invalid_field="conversation_id"
            )
...
if __name__ == "__main__":
    ...
    state = manager.create_initial_state(
        ...
        context={"customer_id": "test_user", "customer_metadata": {"plan": "free"}},
    )
    ...
    state = manager.update_state(
        state, {"primary_intent": "billing_upgrade", "intent_confidence": 0.95, "turn_count": 1}
    )
    print("✓ State updated")
