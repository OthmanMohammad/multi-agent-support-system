...
class TrialOptimizer(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        conversion_probability = self._calculate_conversion_probability(
            engagement_level, conversion_signals, at_risk_indicators, trial_data
        )

        # Determine intervention strategy
        intervention = self._determine_intervention(
            engagement_level, conversion_probability, trial_data
        )
        ...
        recommendations = self._generate_recommendations(
            engagement_level, conversion_probability, intervention, trial_data, customer_metadata
        )

        # Search KB for trial optimization best practices
        kb_results = await self.search_knowledge_base(
            f"trial optimization {engagement_level} engagement", category="sales", limit=3
        )
    ...
    def _analyze_trial_usage(self, trial_data: dict, customer_metadata: dict) -> dict[str, Any]:
    ...
    def _detect_engagement_level(self, trial_data: dict) -> str:
        ...
        trial_data.get("data_uploaded", False)
        trial_data.get("team_invited", False)

        # Check thresholds in order
        if (
            logins_per_week >= self.ENGAGEMENT_THRESHOLDS["high"]["logins_per_week"]
            and features_used >= self.ENGAGEMENT_THRESHOLDS["high"]["features_used"]
        ):
            return "high"
        elif (
            logins_per_week >= self.ENGAGEMENT_THRESHOLDS["medium"]["logins_per_week"]
            and features_used >= self.ENGAGEMENT_THRESHOLDS["medium"]["features_used"]
        ):
            return "medium"
        elif (
            logins_per_week >= self.ENGAGEMENT_THRESHOLDS["low"]["logins_per_week"]
            or features_used >= self.ENGAGEMENT_THRESHOLDS["low"]["features_used"]
        ):
            return "low"
        else:
            return "at_risk"
    ...
    def _determine_intervention(
        self, engagement_level: str, conversion_probability: float, trial_data: dict
    ) -> dict[str, Any]:
        ...
        for milestone, details in self.TRIAL_MILESTONES.items():
            ...
            if days_in_trial >= day_number:
                ...
                progress[milestone] = {
                    ...
                    "completion_rate": min(goals_met / total_goals, 1.0)
                    if total_goals > 0
                    else 0.0,
    ...
    def _generate_recommendations(
        ...
        if conversion_probability > 0.7:
            recommendations.append(
                "HIGH PRIORITY: Strong conversion candidate - fast-track to close"
            )
        ...
        days_remaining = trial_data.get("trial_length_days", 14) - trial_data.get(
            "days_in_trial", 0
        )
        if days_remaining <= 3:
