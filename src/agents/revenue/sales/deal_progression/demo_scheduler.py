...
class DemoScheduler(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        preparation_plan = self._generate_preparation_plan(
            demo_type,
            customer_metadata
        )
        ...
        pre_demo_briefing = self._create_pre_demo_briefing(
            demo_type,
            customer_metadata
        )
    ...
    def _determine_demo_type(
        ...
        if "executive" in message_lower or customer_metadata.get("title", "").lower() in ["ceo", "cto", "cfo"]:
            return "executive"
    ...
    def _find_available_slots(
        ...
        for day_offset in range(1, 15):
            ...
            if "mid_morning" in preferences["preferred_times"] or "morning" in preferences["preferred_times"]:
                slots.append({
                    "datetime": check_date.replace(hour=10, minute=30, second=0),
                    "preference": "high"
                })
            if "mid_afternoon" in preferences["preferred_times"] or "afternoon" in preferences["preferred_times"]:
                slots.append({
        ...
        # Return top 5 slots
        return sorted(slots, key=lambda x: (x["preference"] == "high", x["datetime"]), reverse=True)[:5]
    ...
    async def _generate_scheduling_response(
        ...
        for i, slot in enumerate(available_slots[:3], 1):
            dt = slot["datetime"]
            slots_context += f"{i}. {dt.strftime('%A, %B %d at %I:%M %p')} (Preference: {slot['preference']})\n"
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        print(f"Preparation Time: {result1['preparation_plan']['preparation_time_minutes']} minutes")
        ...
        print(f"Preparation Time: {result3['preparation_plan']['preparation_time_minutes']} minutes")
