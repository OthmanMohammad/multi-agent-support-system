...
class IntegrationObjectionHandler(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        resolution_confidence = self._calculate_resolution_confidence(
            integration_analysis, objection_severity, api_info
        )

        # Determine escalation need
        needs_escalation = self._check_escalation_needed(
            objection_severity, integration_analysis, resolution_confidence
        )
    ...
    def _identify_integrations(self, message: str) -> list[str]:
        ...
        for integration_key, integration_data in self.NATIVE_INTEGRATIONS.items():
            if (
                integration_key in message_lower
                or integration_data["name"].lower() in message_lower
            ):
        ...
        if not mentioned:
            if any(
                term in message_lower for term in ["integrate", "integration", "connect", "api"]
            ):
    ...
    def _analyze_integration_availability(
        self, requested_integrations: list[str]
    ) -> dict[str, Any]:
        ...
        for integration in requested_integrations:
            if integration in self.NATIVE_INTEGRATIONS:
                analysis["native_available"].append(
                    {"name": integration, "data": self.NATIVE_INTEGRATIONS[integration]}
                )
    ...
    def _get_api_info(self, integration_analysis: dict[str, Any]) -> dict[str, Any] | None:
    ...
    def _get_third_party_options(
        ...
        if integration_analysis["zapier_available"] or integration_analysis["custom_needed"]:
            return [self.THIRD_PARTY_PLATFORMS["zapier"], self.THIRD_PARTY_PLATFORMS["make"]]
        return []
    ...
    def _calculate_resolution_confidence(
        self, integration_analysis: dict[str, Any], severity: str, api_info: dict | None
    ) -> float:
    ...
    def _check_escalation_needed(
        self, severity: str, integration_analysis: dict[str, Any], confidence: float
    ) -> bool:
    ...
    async def _generate_integration_response(
        ...
        if api_info:
            ...
            api_context += (
                f"  - Authentication: {', '.join(api_info['rest_api']['authentication'])}\n"
            )
