...
class TrainingSeller(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        recommended_package = self._recommend_package(training_needs, customer_metadata)

        # Calculate productivity ROI
        roi_analysis = self._calculate_training_roi(recommended_package, customer_metadata)

        # Build training curriculum preview
        curriculum = self._build_curriculum_preview(recommended_package, customer_metadata)

        # Generate success metrics
        success_metrics = self._project_success_metrics(recommended_package, customer_metadata)

        # Search KB for training resources
        kb_results = await self.search_knowledge_base(
            "training onboarding feature adoption", category="monetization", limit=2
        )
    ...
    def _identify_training_needs(self, customer_metadata: dict) -> dict[str, Any]:
        ...
        needs = {"has_need": False, "need_score": 0.0, "signals_met": [], "specific_needs": []}
    ...
    def _recommend_package(self, training_needs: dict, customer_metadata: dict) -> str:
    ...
    def _calculate_training_roi(self, package: str, customer_metadata: dict) -> dict[str, Any]:
        ...
        ticket_reduction = (
            current_support_tickets * self.PRODUCTIVITY_METRICS["support_ticket_reduction"]
        )
        ...
        onboarding_hours_saved = (
            new_users_per_year * 8 * self.PRODUCTIVITY_METRICS["onboarding_time_reduction"]
        )
        ...
        # Calculate ROI
        roi_percentage = (
            ((total_annual_value - package_cost) / package_cost) * 100 if package_cost > 0 else 0
        )
    ...
    def _build_curriculum_preview(self, package: str, customer_metadata: dict) -> dict[str, Any]:
        ...
        if package == "enterprise_training":
            topics = (
                base_topics
                + advanced_topics
                + ["Train-the-trainer certification", "Change management strategies"]
            )
    ...
    def _project_success_metrics(self, package: str, customer_metadata: dict) -> dict[str, Any]:
