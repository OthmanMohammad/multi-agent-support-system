...
class AdoptionTracker(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        adoption_analysis = self._analyze_add_on_adoption(
            current_add_ons,
            customer_metadata
        )
        ...
        # Identify underutilized add-ons
        underutilized = self._identify_underutilized_add_ons(
            adoption_analysis,
            health_scores
        )
        ...
        revenue_metrics = self._calculate_addon_revenue_metrics(
            current_add_ons,
            customer_metadata
        )
        ...
        kb_results = await self.search_knowledge_base(
            "add-on adoption feature utilization",
    ...
    def _analyze_add_on_adoption(
        ...
        for add_on_id in add_ons:
            ...
            adoption_percentage = (threshold_met_count / total_thresholds * 100) if total_thresholds > 0 else 0
...
"""
        ...
        if health_scores:
            ...
            for add_on_id, health in health_scores.items():
                health_context += f"- {add_on_id}: {health['overall_health']:.0f}/100 ({health['status']})\n"
