...
class PlanUpgrade(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        upgrade_signals = self._identify_upgrade_signals(
            current_plan,
            customer_metadata
        )

        # Determine target plan
        target_plan = self._determine_target_plan(
            current_plan,
            upgrade_signals
        )

        # Calculate upgrade value
        upgrade_value = self._calculate_upgrade_value(
            current_plan,
            target_plan,
            customer_metadata
        )
        ...
        comparison = self._build_plan_comparison(
            current_plan,
            target_plan
        )

        # Generate feature gap analysis
        feature_gaps = self._analyze_feature_gaps(
            current_plan,
            target_plan,
            customer_metadata
        )
        ...
        kb_results = await self.search_knowledge_base(
            f"plan upgrade {target_plan} features benefits",
    ...
    def _calculate_upgrade_value(
        ...
        feature_value = advanced_features_count * self.VALUE_FACTORS["productivity_per_advanced_feature"]
        ...
        # Calculate ROI
        roi_percentage = ((total_annual_value - annual_increase) / annual_increase) * 100 if annual_increase > 0 else 0
        ...
        return {
            ...
            "payback_months": round(annual_increase / (total_annual_value / 12), 1) if total_annual_value > 0 else 0,
            "value_per_dollar": round(total_annual_value / annual_increase, 2) if annual_increase > 0 else 0
        }
