...
class PricingExperiment(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        if experiment_mode == "design":
            ...
            kb_results = await self.search_knowledge_base(
                "pricing experiment A/B testing optimization",
            ...
            # Search KB
            kb_results = await self.search_knowledge_base(
                "pricing optimization rollout strategy",
                category="monetization",
                limit=2
            )
            state["kb_results"] = kb_results
    ...
    def _design_experiment(self, customer_metadata: Dict) -> Dict[str, Any]:
        ...
        if experiment_type == "price_point_test":
            variants = [
                ...
                {"name": "Lower Price", "price": current_price * 0.85, "description": "15% price reduction"},
                {"name": "Higher Price", "price": current_price * 1.15, "description": "15% price increase"}
            ]
        elif experiment_type == "discount_test":
            variants = [
                {"name": "Control", "discount": 0, "description": "No discount"},
                {"name": "10% Discount", "discount": 0.10, "description": "10% promotional discount"},
                {"name": "20% Discount", "discount": 0.20, "description": "20% promotional discount"}
            ]
    ...
    def _calculate_sample_size(self, experiment: Dict) -> Dict[str, Any]:
        ...
        sample_size_per_variant = int(
            (16 * baseline_conversion * (1 - baseline_conversion)) /
    ...
    def _create_experiment_timeline(self, experiment: Dict) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "activities": ["Start with 25% traffic", "Monitor for issues", "Verify data collection"]
            },
            {
                "phase": "Full test",
                "week": 2,
                "start_date": (start_date + timedelta(weeks=2)).isoformat(),
                "activities": ["Scale to 100% traffic", "Monitor metrics", "Check for significance"]
            },
            {
                "phase": "Analysis",
                "week": duration_weeks,
    ...
    def _perform_statistical_analysis(self, results: Dict) -> Dict[str, Any]:
        ...
        for variant in results["variants"][1:]:
            ...
            ) if control["revenue_per_customer"] > 0 else 0
            ...
            analysis["comparisons"].append({
                ...
                "recommendation": "Roll out" if is_significant and conversion_lift > 0 else "Keep testing" if not is_significant else "Reject"
            })
    ...
    def _create_rollout_plan(self, winner: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "phases": [
                {"phase": 1, "percentage": 25, "duration_days": 7, "description": "25% rollout with monitoring"},
                {"phase": 2, "percentage": 50, "duration_days": 7, "description": "50% rollout if stable"},
