...
class ValueMetricOptimizer(BaseAgent):
    ...
    EVALUATION_CRITERIA = {
        "value_correlation": {"weight": 0.30, "description": "How well metric tracks actual value"},
        "predictability": {"weight": 0.20, "description": "How predictable for customers"},
        "simplicity": {"weight": 0.15, "description": "How easy to understand and explain"},
        "growth_alignment": {"weight": 0.20, "description": "Metric grows with customer success"},
        "operational_ease": {"weight": 0.15, "description": "How easy to implement and track"},
    }
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        metric_evaluation = self._evaluate_alternative_metrics(value_drivers, customer_metadata)

        # Recommend optimal metric
        recommended_metric = self._recommend_optimal_metric(
            metric_evaluation, current_metric_analysis
        )

        # Design pricing model
        pricing_model = self._design_pricing_model(recommended_metric, customer_metadata)

        # Calculate transition impact
        transition_impact = self._calculate_transition_impact(
            current_metric_analysis, recommended_metric, customer_metadata
        )

        # Generate implementation plan
        implementation_plan = self._generate_implementation_plan(
            recommended_metric, transition_impact
        )

        # Search KB for value metric resources
        kb_results = await self.search_knowledge_base(
            "value-based pricing value metrics optimization", category="monetization", limit=2
        )
    ...
    def _analyze_current_metric(self, customer_metadata: dict) -> dict[str, Any]:
        ...
        effectiveness_score = (
            customer_satisfaction * 0.40 + revenue_predictability * 0.30 + (1 - churn_rate) * 0.30
        ) * 100
    ...
    def _identify_value_drivers(self, customer_metadata: dict) -> dict[str, Any]:
        ...
        customer_metadata.get("key_features_used", [])
    ...
    def _evaluate_alternative_metrics(
        self, value_drivers: dict, customer_metadata: dict
    ) -> list[dict[str, Any]]:
        ...
        for metric_name in candidate_metrics:
            ...
            scores = {
                ...
                "growth_alignment": 0.85
                if metric_name in ["transactions", "outcomes", "api_calls"]
                else 0.70,
    ...
    def _recommend_optimal_metric(
        self, evaluations: list[dict], current_analysis: dict
    ) -> dict[str, Any]:
    ...
    def _design_pricing_model(self, metric: dict, customer_metadata: dict) -> dict[str, Any]:
    ...
    def _calculate_transition_impact(
        self, current: dict, recommended: dict, customer_metadata: dict
    ) -> dict[str, Any]:
        ...
        annual_revenue_impact = (
            current_arr * expansion_improvement + current_arr * retention_improvement
        )
        ...
        return {
            ...
            "revenue_impact_percentage": round(
                (annual_revenue_impact / current_arr * 100) if current_arr > 0 else 0, 2
            ),
    ...
    def _generate_implementation_plan(self, metric: dict, impact: dict) -> list[dict[str, Any]]:
