...
class DiscountManager(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        discount_request = self._extract_discount_request(
            customer_metadata
        )

        # Validate discount justification
        justification_analysis = self._validate_justification(
            discount_request
        )
        ...
        if decision["requires_escalation"]:
            ...
            state["escalation_reason"] = f"Discount approval needed from {approval_tier['approver']}"
    ...
    def _validate_justification(self, request: Dict) -> Dict[str, Any]:
        ...
        if matched_reasons:
            ...
            strength = "strong" if avg_weight >= 0.80 else "moderate" if avg_weight >= 0.60 else "weak"
        else:
            strength = "none"
            avg_weight = 0
    ...
    def _identify_red_flags(
        ...
        if customer_metadata.get("demo_completed", True) == False:
    ...
    def _calculate_discount_impact(
        self,
        request: Dict,
        customer_metadata: Dict
    ) -> Dict[str, Any]:
        ...
        annual_deal_size = deal_size * (12 / term_months) if term_months > 0 else deal_size
        annual_discount_cost = discount_amount * (12 / term_months) if term_months > 0 else discount_amount

        # Calculate total contract impact
        total_contract_value = deal_size
        ...
        if justification["strength"] in ["strong", "moderate"] and len(red_flags) <= 1:
            return {
                ...
                "conditions": [
                    ...
                    "Competitor evidence required" if "competitive" in request["justification"].lower() else None
                ],
