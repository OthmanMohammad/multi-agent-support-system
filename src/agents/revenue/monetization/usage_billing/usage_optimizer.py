...
class UsageOptimizer(BaseAgent):
    ...
    OPTIMIZATION_STRATEGIES = {
        ...
        "seats_active": {
            ...
            "low_usage_tips": ["Monitor seat utilization regularly", "Plan for growth efficiently"],
        },
        "tickets_created": {
            "high_usage_tips": [
            ...
            "low_usage_tips": ["Monitor ticket trends", "Identify training opportunities"],
        },
    }
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        usage_analysis = self._analyze_usage_patterns(usage_data, customer_metadata)

        # Identify inefficiencies
        inefficiencies = self._identify_inefficiencies(
            usage_data, usage_analysis, customer_metadata
        )

        # Generate optimization recommendations
        recommendations = self._generate_recommendations(
            usage_data, inefficiencies, customer_metadata
        )

        # Calculate potential savings
        potential_savings = self._calculate_potential_savings(
            recommendations, usage_data, customer_metadata
        )
        ...
        kb_results = await self.search_knowledge_base(
            "usage optimization cost reduction", category="monetization", limit=3
        )
    ...
    def _analyze_usage_patterns(self, usage_data: dict, customer_metadata: dict) -> dict[str, Any]:
        """Analyze usage patterns to identify trends"""
        analysis = {"usage_levels": {}, "trends": {}, "efficiency_score": 0}
        ...
        analysis["efficiency_score"] = (
            max(0, 100 - (high_count / total_count * 30)) if total_count > 0 else 100
        )

        return analysis

    def _identify_inefficiencies(
        self, usage_data: dict, usage_analysis: dict, customer_metadata: dict
    ) -> list[dict[str, Any]]:
    ...
    def _generate_recommendations(
        self, usage_data: dict, inefficiencies: list[dict], customer_metadata: dict
    ) -> list[dict[str, Any]]:
    ...
    def _calculate_potential_savings(
        self, recommendations: list[dict], usage_data: dict, customer_metadata: dict
    ) -> dict[str, Any]:
        """Calculate potential cost savings from recommendations"""
        savings = {"by_metric": {}, "total_monthly_savings": 0.0, "total_annual_savings": 0.0}

        overage_pricing = customer_metadata.get("overage_pricing", {})
        customer_metadata.get("base_plan_cost", 0)
        ...
        for rec in recommendations:
            ...
            if metric not in savings["by_metric"]:
                savings["by_metric"][metric] = {"monthly_savings": 0.0, "annual_savings": 0.0}
    ...
    def _prioritize_recommendations(
        self, recommendations: list[dict], potential_savings: dict
    ) -> list[dict[str, Any]]:
