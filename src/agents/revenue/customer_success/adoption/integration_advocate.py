...
class IntegrationAdvocateAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        integration_analysis = self._analyze_integration_state(
            current_integrations, integration_usage, tech_stack
        )

        # Identify integration opportunities
        opportunities = self._identify_integration_opportunities(
            tech_stack, current_integrations, customer_metadata
        )

        # Calculate integration value
        value_analysis = self._calculate_integration_value(
            opportunities, integration_analysis, customer_metadata
        )
        ...
        roadmap = self._create_integration_roadmap(recommendations, integration_analysis)

        # Format response
        response = self._format_integration_report(
            integration_analysis, opportunities, value_analysis, recommendations, roadmap
        )
    ...
    def _analyze_integration_state(
        ...
        integration_health_pct = (
            (active_integrations / integration_count * 100) if integration_count > 0 else 0
        )
        ...
        integration_score = self._calculate_integration_score(
            integration_count, integrations_by_category, integration_health_pct, avg_syncs_per_day
        )
        ...
        # Identify gaps
        integration_gaps = self._identify_integration_gaps(integrations_by_category, tech_stack)
    ...
    def _categorize_integrations(
        self, current_integrations: list[dict[str, Any]]
    ) -> dict[str, list[str]]:
        """Categorize existing integrations."""
        categorized = {category: [] for category in self.INTEGRATION_CATEGORIES}
    ...
    def _calculate_integration_score(
        ...
        categories_used = sum(
            1 for integrations in integrations_by_category.values() if integrations
        )
    ...
    def _identify_integration_gaps(
        self, integrations_by_category: dict[str, list[str]], tech_stack: dict[str, Any]
    ) -> list[str]:
    ...
    def _identify_integration_opportunities(
        ...
        integrated_tools = {
            integration.get("tool_name", "").lower() for integration in current_integrations
        }
        ...
        if not any(opp["category"] == "crm" for opp in opportunities):
            ...
            recommended_crm = (
                "Salesforce" if customer_metadata.get("plan") == "enterprise" else "HubSpot"
            )
        ...
        opportunities.sort(key=lambda x: (priority_order.get(x["priority"], 3), -x["stickiness"]))
    ...
    def _generate_use_case(self, category: str, industry: str, tool: str) -> str:
        ...
        use_cases = {
            ...
            "communication": "Send notifications and updates directly to team channels",
            "project_management": f"Create and update {industry} tasks from workflow events",
            "storage": "Attach files and documents seamlessly to records",
            "analytics": f"Track {industry} metrics and user behavior",
            "customer_support": "Link support tickets to customer records",
            "marketing": f"Sync {industry} leads and campaign data",
            "development": "Link code commits and deployments to projects",
    ...
    def _calculate_integration_value(
        ...
        ecosystem_value_score = min(
            (critical_count * 20)
            + (high_count * 10)
            + (integration_analysis["integration_count"] * 5),
            100,
        )
    ...
    def _estimate_setup_time(self, complexity: str) -> str:
        ...
        time_map = {"low": "15-30 minutes", "medium": "1-2 hours", "high": "3-5 hours"}
        return time_map.get(complexity, "1-2 hours")

    def _create_integration_roadmap(
        self, recommendations: list[dict[str, Any]], integration_analysis: dict[str, Any]
    ) -> dict[str, list[str]]:
        ...
        for rec in recommendations:
            if rec["priority"] == "critical" or (
                rec["priority"] == "high" and rec["setup_complexity"] == "low"
            ):
                roadmap["week_1"].append(f"Integrate {rec['tool']} ({rec['category']})")

        # Weeks 2-4: High priority and medium complexity
        for rec in recommendations:
            if rec["priority"] == "high" and rec["setup_complexity"] == "medium":
                roadmap["week_2_4"].append(f"Integrate {rec['tool']} ({rec['category']})")

        # Months 2-3: Remaining and high complexity
        for rec in recommendations:
            if rec["priority"] in ["medium", "low"] or rec["setup_complexity"] == "high":
                roadmap["month_2_3"].append(f"Integrate {rec['tool']} ({rec['category']})")
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        state1["entities"] = {
            "tech_stack": {"tools": ["Salesforce", "Slack", "Google Drive", "Jira", "Zendesk"]},
            "current_integrations": [{"tool_name": "Slack", "status": "active", "sync_errors": 0}],
            "integration_usage": {"avg_syncs_per_day": 15, "data_flowing": True},
        }
        ...
        state2["entities"] = {
            ...
            "integration_usage": {"avg_syncs_per_day": 250, "data_flowing": True},
        }
