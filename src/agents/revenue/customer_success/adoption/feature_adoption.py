...
class FeatureAdoptionAgent(BaseAgent):
    ...
    def _analyze_feature_adoption(
        ...
        high_value_unused = self._identify_high_value_unused(
            unused_features,
            customer_metadata
        )
        ...
        return {
            ...
            "adoption_percentage": int((len(features_used) / total_features * 100)) if total_features > 0 else 0,
        ...
        for feature in unused_features:
            ...
            if value_score >= 50:  # Advanced or Core features
                high_value.append({
                    ...
                    "potential_impact": self._estimate_feature_impact(feature, customer_metadata)
                })
    ...
    def _calculate_adoption_velocity(self, usage_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        previous_month_features = usage_data.get("previous_month_features_count", current_month_features)

        if previous_month_features > 0:
            velocity_pct = ((current_month_features - previous_month_features) / previous_month_features) * 100
        else:
            velocity_pct = 0

        trend = "accelerating" if velocity_pct > 10 else "steady" if velocity_pct >= -5 else "declining"
    ...
    def _generate_feature_recommendations(
        ...
        for feature_data in adoption_analysis["high_value_unused"][:5]:
            recommendations.append({
                ...
                "priority": self.FEATURE_TIERS.get(feature_data["tier"], {}).get("adoption_priority", "medium"),
                "value_score": feature_data["value_score"],
                "reason": f"{feature_data['potential_impact']}",
                "suggested_use_case": self._suggest_use_case(feature_data["feature"], customer_metadata),
        ...
        recommendations.sort(key=lambda x: (priority_order.get(x["priority"], 3), -x["value_score"]))
    ...
    def _calculate_roi_opportunity(
        ...
        high_priority_count = sum(1 for rec in recommendations if rec["priority"] in ["critical", "high"])
        estimated_time_savings_hours = high_priority_count * 10  # 10 hours per month per feature

        # Calculate retention impact
        adoption_score = adoption_analysis["overall_adoption_score"]
        retention_impact = "high" if adoption_score < 50 else "medium" if adoption_score < 75 else "low"
    ...
    def _format_adoption_report(
        ...
        score_emoji = "????" if score >= 75 else "???" if score >= 50 else "??????" if score >= 25 else "????"
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        state1["entities"] = {
            ...
            "available_features": [
                "basic_reporting", "user_management", "advanced_analytics",
                "automation_builder", "api_access", "custom_dashboards",
                "collaboration_tools", "integration_hub"
            ]
        }
        ...
        state2["entities"] = {
            "usage_data": {
                "features_used": ["basic_reporting", "advanced_analytics", "automation_builder",
                                 "custom_dashboards", "collaboration_tools"],
                "feature_usage_frequency": {
                    "basic_reporting": 45,
            ...
            "available_features": [
                "basic_reporting", "advanced_analytics", "automation_builder",
                "custom_dashboards", "collaboration_tools", "api_access"
            ]
        }
