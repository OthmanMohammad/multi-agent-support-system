...
class UsageBasedExpansionAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        usage_analysis = self._analyze_usage_vs_limits(usage_data, contract_data)

        # Predict overages
        overage_prediction = self._predict_overages(usage_analysis, usage_data, contract_data)

        # Calculate expansion ROI
        expansion_roi = self._calculate_expansion_roi(
            overage_prediction, usage_analysis, contract_data
        )

        # Generate expansion recommendations
        recommendations = self._generate_expansion_recommendations(
            usage_analysis, overage_prediction, expansion_roi, contract_data
        )

        # Create action plan
        action_plan = self._create_action_plan(usage_analysis, overage_prediction, recommendations)

        # Format response
        response = self._format_expansion_report(
            usage_analysis, overage_prediction, expansion_roi, recommendations, action_plan
        )
    ...
    def _analyze_usage_vs_limits(
        self, usage_data: dict[str, Any], contract_data: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        usage_velocity = self._calculate_usage_velocity(
            current_usage, usage_data.get("previous_month_usage", {})
        )
    ...
    def _calculate_usage_velocity(
        self, current_usage: dict[str, int], previous_usage: dict[str, int]
    ) -> dict[str, Any]:
        ...
        avg_velocity = (
            round(total_growth / max(metrics_growing, 1), 1) if metrics_growing > 0 else 0
        )
    ...
    def _predict_overages(
        ...
        contract_data.get("usage_limits", {})
        usage_data.get("current_usage", {})
    ...
    def _calculate_expansion_roi(
        ...
        payback_months = expansion_cost / monthly_overage if monthly_overage > 0 else 999
    ...
    def _generate_expansion_recommendations(
        ...
        if expansion_roi["recommendation"] in ["strongly_recommended", "recommended"]:
            tier_config = self.TIER_RECOMMENDATIONS.get(
                f"overage_risk_{risk_level}", self.TIER_RECOMMENDATIONS["overage_risk_low"]
            )
...
"""
        ...
        if overage_prediction["overage_predictions"]:
            ...
            report += (
                f"**Annual Projection:** ${overage_prediction['annual_overage_projection']:,}\n"
            )
        ...
        report += (
            f"**Recommendation:** {expansion_roi['recommendation'].replace('_', ' ').title()}\n"
        )
    ...
    async def test():
        ...
        state1 = create_initial_state(
            ...
            context={
                ...
                "customer_metadata": {"plan": "professional"},
            },
        )
        state1["entities"] = {
            "usage_data": {
        ...
        state2 = create_initial_state(
            "Check usage levels",
            context={"customer_id": "cust_healthy", "customer_metadata": {"plan": "enterprise"}},
        )
        state2["entities"] = {
            "usage_data": {
                "current_usage": {"api_calls": 500000, "storage": 300, "users": 60},
                "previous_month_usage": {"api_calls": 480000, "storage": 290, "users": 58},
                "days_in_current_period": 15,
            },
            "contract_data": {
                "tier": "enterprise",
                "contract_value": 100000,
                "usage_limits": {"api_calls": 2000000, "storage": 1000, "users": 100},
                "overage_rates": {"api_calls": 0.04, "storage": 40, "users": 80},
            },
        }
