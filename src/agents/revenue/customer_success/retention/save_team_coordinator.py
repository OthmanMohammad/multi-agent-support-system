...
class SaveTeamCoordinatorAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        save_assessment = self._assess_save_team_need(
            risk_data,
            contract_data,
            churn_indicators
        )
        ...
        # Assemble save team
        save_team = self._assemble_save_team(
            save_assessment,
            customer_metadata,
            contract_data
        )
    ...
    def _determine_save_team_requirement(
        ...
        if days_to_renewal <= 14 and risk_level != "low":
            return True
    ...
    def _categorize_churn_indicators(
        ...
        for indicator in churn_indicators:
            ...
            if "feature" in indicator_type or "product" in indicator_type or "bug" in indicator_type:
            ...
            elif "payment" in indicator_type or "price" in indicator_type or "billing" in indicator_type:
                categorized["financial"].append(indicator_desc)
            elif "usage" in indicator_type or "login" in indicator_type or "engagement" in indicator_type:
    ...
    def _assemble_save_team(
        ...
        team_size = self.URGENCY_LEVELS[urgency]["team_size"]
        ...
        team_members.extend([
            {"role": "csm", "name": "Customer Success Manager", "responsibilities": ["Lead save team", "Customer relationship"]},
            {"role": "cs_manager", "name": "CS Manager", "responsibilities": ["Strategic oversight", "Escalation support"]},
            {"role": "account_exec", "name": "Account Executive", "responsibilities": ["Contract negotiation", "Commercial terms"]}
        ])
        ...
        if exec_involvement == "required" or (exec_involvement == "recommended" and contract_data.get("contract_value", 0) > 75000):
            team_members.append({
                "role": "exec_sponsor",
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        state1["entities"] = {
            ...
            "churn_indicators": [
                {"type": "product_feature_gap", "description": "Missing critical analytics features"},
