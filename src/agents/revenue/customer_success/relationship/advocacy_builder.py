...
class AdvocacyBuilderAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        advocacy_assessment = self._assess_advocacy_readiness(
            engagement_data, business_data, health_data, customer_metadata
        )

        # Identify advocacy opportunities
        advocacy_opportunities = self._identify_advocacy_opportunities(
            advocacy_assessment, engagement_data, customer_metadata
        )

        # Create advocacy request plan
        request_plan = self._create_advocacy_request_plan(advocacy_opportunities, customer_metadata)
        ...
        # Create action plan
        action_plan = self._create_advocacy_action_plan(advocacy_assessment, advocacy_opportunities)

        # Format response
        response = self._format_advocacy_report(
            advocacy_assessment, advocacy_opportunities, request_plan, story_ideas, action_plan
        )
        ...
        state["recommended_advocacy_type"] = (
            advocacy_opportunities[0]["type"] if advocacy_opportunities else "none"
        )
    ...
    def _assess_advocacy_readiness(
        ...
        is_advocacy_ready = readiness_score >= 70 and nps >= 8 and tenure_days >= 30
    ...
    def _identify_advocacy_opportunities(
        ...
        for advocacy_type, requirements in self.ADVOCACY_TYPES.items():
            if nps >= requirements["nps_min"] and tenure_days >= requirements["tenure_min"]:
                # Calculate fit score
                fit_score = 100
                if advocacy_type in ["case_study", "speaking"] and tier not in [
                    "enterprise",
                    "premium",
                ]:
                    fit_score -= 20
                ...
                opportunity = {
                    ...
                    "priority": self._determine_advocacy_priority(
                        advocacy_type, advocacy_assessment, tier
                    ),
                }
        ...
        opportunities.sort(key=lambda x: (priority_order.get(x["priority"], 3), -x["fit_score"]))

        return opportunities

    def _determine_advocacy_priority(
        self, advocacy_type: str, advocacy_assessment: dict[str, Any], tier: str
    ) -> str:
    ...
    def _create_advocacy_request_plan(
        self, advocacy_opportunities: list[dict[str, Any]], customer_metadata: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        message_template = self._create_advocacy_message_template(
            advocacy_type, top_opportunity["incentive"], customer_metadata
        )
    ...
    def _create_advocacy_message_template(
        self, advocacy_type: str, incentive: str, customer_metadata: dict[str, Any]
    ) -> str:
    ...
    def _generate_success_story_ideas(
        ...
        if roi > 100:
            story_ideas.append(f"How {company_name} achieved {roi}% ROI in {industry}")

        # Adoption success
        story_ideas.append(f"{company_name}'s journey to successful platform adoption")

        # Transformation story
        story_ideas.append(f"Transforming {industry} operations: {company_name}'s story")

        # Efficiency story
        story_ideas.append(f"Streamlining {industry} workflows: A {company_name} case study")

        return story_ideas[:4]

    def _create_advocacy_action_plan(
        self, advocacy_assessment: dict[str, Any], advocacy_opportunities: list[dict[str, Any]]
    ) -> list[dict[str, str]]:
...
"""
        ...
        for dimension, score_val in advocacy_assessment["score_breakdown"].items():
            ...
            report += (
                f"- {dimension.replace('_', ' ').title()}: {int(score_val)}/{max_score} {bar}\n"
            )
