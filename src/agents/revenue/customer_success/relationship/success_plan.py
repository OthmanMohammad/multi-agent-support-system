...
class SuccessPlanAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        self.logger.debug(
            "success_plan_details", customer_id=customer_id, has_existing_plan=bool(existing_plan)
        )

        # Create or update success plan
        if existing_plan:
            success_plan = self._update_success_plan(
                existing_plan, usage_data, engagement_data, business_data
            )
        else:
            success_plan = self._create_success_plan(customer_metadata, business_data)

        # Track progress
        progress_analysis = self._track_plan_progress(
            success_plan, usage_data, engagement_data, business_data
        )

        # Identify blockers
        blockers = self._identify_plan_blockers(success_plan, progress_analysis, usage_data)

        # Calculate value realization
        value_analysis = self._calculate_value_realization(success_plan, usage_data, business_data)

        # Generate recommendations
        recommendations = self._generate_plan_recommendations(
            success_plan, progress_analysis, blockers
        )

        # Create action plan
        action_plan = self._create_success_action_plan(success_plan, progress_analysis, blockers)

        # Format response
        response = self._format_success_plan_report(
            success_plan, progress_analysis, value_analysis, blockers, recommendations, action_plan
        )
    ...
    def _define_business_objectives(
        self, tier: str, industry: str, business_data: dict[str, Any]
    ) -> list[dict[str, Any]]:
    ...
    def _create_milestones(
        self, objectives: list[dict[str, Any]], tier: str
    ) -> list[dict[str, Any]]:
    ...
    def _set_value_metrics(
        self, objectives: list[dict[str, Any]], business_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _update_success_plan(
        ...
        for criterion in existing_plan.get("success_criteria", []):
            ...
            if metric == "User Adoption Rate":
                ...
                feature_adoption = (
                    int(features_used / total_features * 100) if total_features > 0 else 0
                )
        ...
        for milestone in existing_plan.get("milestones", []):
            if milestone["status"] == "pending":
                ...
                milestone_met = self._check_milestone_completion(
                    milestone, usage_data, engagement_data
                )
    ...
    def _check_milestone_completion(
        self, milestone: dict[str, Any], usage_data: dict[str, Any], engagement_data: dict[str, Any]
    ) -> bool:
    ...
    def _calculate_value_realization(
        ...
        time_savings_pct = (
            int(time_saved_hours / target_time_savings * 100) if target_time_savings > 0 else 0
        )

        target_roi = value_metrics.get("target_roi_multiple", 3.0)
        current_roi_multiple = (
            estimated_value_realized / contract_value if contract_value > 0 else 0
        )
    ...
    def _generate_plan_recommendations(
        ...
        if blockers:
            recommendations.append(
                f"Address {len(blockers)} identified blockers to get plan back on track"
            )
    ...
    def _create_success_action_plan(
        ...
        pending_milestones = [
            m for m in success_plan.get("milestones", []) if m["status"] == "pending"
        ]
    ...
    def _format_success_plan_report(
        ...
        status_emoji = {"on_track": "???", "needs_attention": "??????", "at_risk": "????"}
...
"""
        ...
        for criterion in success_plan.get("success_criteria", []):
            status_icon = (
                "???"
                if criterion.get("status") == "on_track"
                else "??????"
                if criterion.get("status") == "needs_attention"
                else "???"
            )
    ...
    async def test():
        ...
        state1 = create_initial_state(
            ...
            context={
                ...
                "customer_metadata": {"tier": "enterprise", "industry": "healthcare"},
            },
        )
        state1["entities"] = {
            "business_data": {"contract_value": 120000},
            "usage_data": {
                "active_users": 0,
                "total_users": 100,
                "features_used": [],
        ...
        existing_plan = {
            ...
            "value_metrics": {"target_roi_multiple": 3.0, "target_time_savings_hours": 500},
            "status": "active",
        }

        state2 = create_initial_state(
            "Update success plan",
            context={"customer_id": "cust_premium_002", "customer_metadata": {"tier": "premium"}},
        )
        state2["entities"] = {
            "success_plan": existing_plan,
            "business_data": {"contract_value": 60000},
            "usage_data": {
                "active_users": 65,
                "total_users": 75,
