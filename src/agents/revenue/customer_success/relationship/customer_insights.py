...
class CustomerInsightsAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        # Analyze trends
        trend_analysis = self._analyze_trends(usage_data, engagement_data, historical_data)

        # Identify anomalies
        anomalies = self._detect_anomalies(usage_data, engagement_data, historical_data)

        # Generate predictions
        predictions = self._generate_predictions(insights, trend_analysis, customer_metadata)

        # Create benchmarking comparison
        benchmarking = self._create_benchmarking_analysis(
            usage_data, engagement_data, customer_metadata
        )

        # Generate recommendations
        recommendations = self._generate_insight_recommendations(insights, anomalies, predictions)

        # Create action plan
        action_plan = self._create_insights_action_plan(insights, predictions, anomalies)
    ...
    def _analyze_usage_patterns(
        self, usage_data: dict[str, Any], historical_data: dict[str, Any]
    ) -> list[dict[str, Any]]:
    ...
    def _analyze_engagement_patterns(
        self, engagement_data: dict[str, Any], historical_data: dict[str, Any]
    ) -> list[dict[str, Any]]:
    ...
    def _analyze_value_realization(
        ...
        if estimated_value < contract_value * 0.5:
            insights.append(
                {
                    ...
                    "insight": "Value realization below 50% of contract value - ROI gap",
                    "priority": "high",
            ...
            insights.append(
                {
                    "category": "success_factor",
                    "insight": "Strong value realization at 2x+ ROI - success story candidate",
                    "priority": "low",
    ...
    def _analyze_trends(
        ...
        usage_change = (
            ((current_users - previous_users) / previous_users * 100) if previous_users > 0 else 0
        )
        usage_trend = (
            "increasing" if usage_change > 10 else "stable" if usage_change >= -5 else "declining"
        )
        ...
        engagement_trend = (
            "improving" if nps_change > 1 else "stable" if nps_change >= -1 else "declining"
        )
    ...
    def _create_benchmarking_analysis(
        ...
        return {
            ...
            if actual_adoption > benchmark["adoption"]
            else "below",
        }
    ...
    def _generate_insight_recommendations(
        ...
        if critical:
            recommendations.append(
                "Address critical risk indicators immediately - account requires urgent attention"
            )

        # Address anomalies
        if anomalies:
            recommendations.append(
                f"Investigate {len(anomalies)} detected anomalies to understand root causes"
            )
        ...
        if opportunities:
            recommendations.append(
                f"Pursue {len(opportunities)} identified expansion opportunities"
            )
    ...
    def _format_insights_report(
        ...
        trend_emoji = {"positive": "????", "stable": "??????", "negative": "????"}
...
"""
        ...
        for i, insight in enumerate(insights[:5], 1):
            ...
            category_icon = (
                "??????"
                if insight["category"] == "risk_indicator"
                else "????"
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        state1["entities"] = {
            ...
            "business_data": {"contract_value": 120000, "contracted_users": 100},
        ...
        state2["entities"] = {
            ...
            "business_data": {"contract_value": 35000, "contracted_users": 50},
            "health_data": {"health_score": 35},
            "historical_data": {
