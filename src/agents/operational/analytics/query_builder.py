...
class QueryBuilderAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        self.logger.debug(
            "query_building_details", nl_query_length=len(nl_query), execute_query=execute_query
        )
...
Please rephrase your request to only query data (SELECT statements only)."""
        ...
        # Format response
        response = self._format_query_response(nl_query, sql_query, explanation, execution_result)
        ...
        self.logger.info(
            "query_building_completed", query_generated=True, is_safe=True, executed=execute_query
        )
...
Return the intent as a structured format."""
    ...
    def _extract_intent_from_response(self, llm_response: str, nl_query: str) -> dict[str, Any]:
    ...
    def _generate_sql_query(self, query_intent: dict[str, Any]) -> str:
        ...
        # Assemble query
        sql_query = (
            f"SELECT {select_clause} FROM {table}{where_clause}{group_by_clause}{limit_clause};"
        )
    ...
    def _explain_query(self, sql_query: str, query_intent: dict[str, Any]) -> str:
...
"""
            ...
            response += (
                "**Note:** Query was generated but not executed. Use execute_query=true to run.\n"
            )
