...
class ReportGeneratorAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        full_report = self._compile_full_report(
            template["name"], sections, period_start or "N/A", period_end or "N/A"
        )
    ...
    def _get_report_template(
        self, report_type: str, custom_sections: list[str] | None
    ) -> dict[str, Any]:
        """Get report template."""
    ...
    def _collect_report_data(
        self, template: dict[str, Any], period_start: str | None, period_end: str | None
    ) -> dict[str, Any]:
        ...
        return {
            "period": {
                "start": period_start
                or (datetime.now(UTC) - timedelta(days=30)).strftime("%Y-%m-%d"),
    ...
    def _generate_report_sections(
        self, template: dict[str, Any], report_data: dict[str, Any]
    ) -> dict[str, str]:
    ...
    def _generate_section_content(self, section_name: str, report_data: dict[str, Any]) -> str:
...
- CSAT: {support.get("csat", 0)}/5.0"""
        ...
        elif section_name in {"highlights", "key_wins"}:
        ...
        elif section_name in {"concerns", "challenges"}:
        ...
        elif section_name in {"financial_performance", "financial_metrics"}:
            financial = report_data.get("financial_metrics", {})
    ...
    def _compile_full_report(
        self, report_name: str, sections: dict[str, str], period_start: str, period_end: str
    ) -> str:
