...
class AccessControllerAgent(BaseAgent):
    ...
    ROLE_PERMISSIONS = {
        ...
        "manager": {"read", "write", "delete", "team_management", "report_access", "user_view"},
        "agent": {"read", "write", "ticket_management", "customer_interaction"},
        "user": {"read", "self_service", "ticket_create"},
        "readonly": {"read"},
        "guest": set(),
    }
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        resource_check = self._validate_resource_access(user_role, resource_type, protection_level)

        # Check MFA requirement
        mfa_check = self._check_mfa_requirement(requested_operation, mfa_verified, protection_level)

        # Evaluate access rules
        access_granted, denial_reason = self._evaluate_access(
            session_check, permission_check, resource_check, mfa_check
        )
        ...
        recommendations = self._generate_recommendations(
            access_granted, denial_reason, user_role, requested_operation
        )

        # Check for security violations
        violations = self._check_security_violations(
            access_granted, user_id, requested_operation, denial_reason
        )
    ...
    def _check_permissions(self, user_role: str, operation: str) -> dict[str, Any]:
        ...
        return {
            ...
            "reason": f"Role '{user_role}' has permission for '{operation}'"
            if has_permission
            else f"Role '{user_role}' lacks permission for '{operation}'",
        }

    def _validate_resource_access(
        self, user_role: str, resource_type: str, protection_level: str
    ) -> dict[str, Any]:
        """
        ...
        return {
            ...
            "protection_level": protection_level,
            "resource_type": resource_type,
            "reason": f"User clearance level {user_clearance} sufficient for {protection_level}"
            if has_access
            else f"User clearance level {user_clearance} insufficient for {protection_level}",
        }

    def _check_mfa_requirement(
        self, operation: str, mfa_verified: bool, protection_level: str
    ) -> dict[str, Any]:
        """
        ...
        Returns:
            MFA check result
        """
        mfa_required = operation in self.MFA_REQUIRED_OPERATIONS or protection_level in [
            "secret",
            "top_secret",
        ]
    ...
    def _generate_recommendations(
        self, access_granted: bool, denial_reason: str | None, user_role: str, operation: str
    ) -> list[str]:
        """
    ...
    def _check_security_violations(
        self, access_granted: bool, user_id: str, operation: str, denial_reason: str | None
    ) -> list[dict[str, Any]]:
        """
        Check for security violations.
...
"""
        ...
        if violations:
            ...
            for violation in violations:
                ...
                report += (
                    f"{severity_icon} [{violation['severity'].upper()}] {violation['message']}\n"
                )
            report += "\n"
