...
class NotificationSenderAgent(BaseAgent):
    ...
    PRIORITY_CONFIGS = {
        "critical": {"retry_attempts": 5, "retry_delay_seconds": 30, "timeout_seconds": 10},
        "high": {"retry_attempts": 3, "retry_delay_seconds": 60, "timeout_seconds": 30},
        "normal": {"retry_attempts": 2, "retry_delay_seconds": 120, "timeout_seconds": 60},
        "low": {"retry_attempts": 1, "retry_delay_seconds": 300, "timeout_seconds": 120},
    }
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        notification_content = await self._parse_notification_content(
            message, notification_type, customer_metadata
        )

        # Determine target channels
        target_channels = self._determine_channels(notification_type, channels, customer_metadata)

        # Prepare channel-specific messages
        channel_messages = self._prepare_channel_messages(
            notification_content, target_channels, notification_type
        )

        # Validate messages for each channel
        validation_results = self._validate_messages(channel_messages, target_channels)

        # Send notifications to all channels
        sent_notifications = await self._send_notifications(
            channel_messages, target_channels, priority, validation_results
        )

        # Setup retry for failed notifications
        retry_config = self._setup_retry_policy(sent_notifications, priority)

        # Log automation action
        automation_log = self._log_automation_action(
            "notification_sent", sent_notifications, customer_metadata
        )

        # Generate response
        response = self._format_notification_response(
            sent_notifications, target_channels, retry_config
        )
    ...
    async def _parse_notification_content(
        self, message: str, notification_type: str, customer_metadata: dict
    ) -> dict[str, Any]:
        """
...
Return JSON with title, body, priority, action_url, and additional_context."""
    ...
    def _determine_channels(
        self, notification_type: str, requested_channels: list[str] | None, customer_metadata: dict
    ) -> list[str]:
        """
        ...
        notification_config = self.NOTIFICATION_TYPES.get(
            notification_type, self.NOTIFICATION_TYPES["info"]
        )
    ...
    def _prepare_channel_messages(
        self, notification_content: dict, target_channels: list[str], notification_type: str
            ...
            if channel == "slack":
                ...
                channel_messages[channel] = {
                    "incident": {
                        "type": "incident",
                        "title": notification_content["title"],
                        "body": {"type": "incident_body", "details": notification_content["body"]},
    ...
    def _validate_messages(
        self, channel_messages: dict[str, dict], target_channels: list[str]
    ) -> dict[str, dict[str, Any]]:
        """
        Validate messages for each channel.
    ...
    async def _send_notifications(
        ...
        for channel in target_channels:
            ...
            notification_id = hashlib.md5(f"{channel}{datetime.now(UTC)}".encode()).hexdigest()[:12]
    ...
    def _setup_retry_policy(self, sent_notifications: list[dict], priority: str) -> dict[str, Any]:
        """
        ...
        Returns:
            Retry configuration
        """
        priority_config = self.PRIORITY_CONFIGS.get(priority, self.PRIORITY_CONFIGS["normal"])

        failed_notifications = [n for n in sent_notifications if n.get("status") != "sent"]
    ...
    def _log_automation_action(
        self, action_type: str, sent_notifications: list[dict], customer_metadata: dict
    ) -> dict[str, Any]:
    ...
    def _format_notification_response(
        self, sent_notifications: list[dict], target_channels: list[str], retry_config: dict
    ) -> str:
