...
class DeduplicatorAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        high_confidence_dupes = [
            d for d in scored_duplicates if d["confidence"] >= self.SIMILARITY_THRESHOLDS["high"]
        ]

        # Prepare merge operations
        merge_operations = self._prepare_merge_operations(high_confidence_dupes, merge_strategy)

        # Execute merges
        merge_results = await self._execute_merges(merge_operations)

        # Log automation action
        automation_log = self._log_automation_action("duplicates_merged", merge_results, {})
...
"""
    ...
    def _prepare_merge_operations(self, duplicates: list[dict], strategy: str) -> list[dict]:
        """Prepare merge operations."""
        operations = []
        for dup in duplicates:
            operations.append(
    ...
    def _log_automation_action(
        self, action_type: str, merge_results: dict, customer_metadata: dict
    ) -> dict:
        """Log automation action."""
        return {
