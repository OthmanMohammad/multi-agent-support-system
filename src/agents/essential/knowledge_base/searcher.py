...
class KBSearcher(BaseAgent):
    ...
    def __init__(self):
        ...
        self.logger.info(
            "kb_searcher_initialized", collection_name=self.vector_store.collection_name
        )
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        # Perform search
        results = self.search(
            query=query, limit=limit, min_similarity=min_similarity, category=category, tags=tags
        )
    ...
    async def search_by_intent(self, intent: str, user_message: str, limit: int = 3) -> list[dict]:
        """
        ...
        self.logger.info("kb_search_by_intent", intent=intent, category=category)

        return self.search(query=user_message, category=category, limit=limit)
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        results = searcher.search(query="billing question", category="billing", limit=3)
        print(f"Found {len(results)} results in 'billing'")

        # Test 3: With state
        print("\nTest 3: Process with state")
        state = create_initial_state(
            message="How do I invite team members?", context={"kb_search_limit": 2}
        )
