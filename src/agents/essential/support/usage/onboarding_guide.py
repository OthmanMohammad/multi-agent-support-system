...
class UsageAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        if kb_results:
            ...
            self.logger.warning("usage_no_kb_articles_found", intent=intent)
        ...
        self.logger.info("usage_response_generated", response_length=len(response), status="active")
    ...
    async def generate_response(
        ...
        self.logger.debug(
            "usage_response_generation_started", intent=intent, kb_articles_count=len(kb_results)
        )
        ...
        self.logger.debug("usage_conversation_context", history_length=len(conversation_history))
...
Provide a helpful how-to guide that takes into account any previous conversation context."""
        ...
        self.logger.debug("usage_llm_response_received", response_length=len(response))
