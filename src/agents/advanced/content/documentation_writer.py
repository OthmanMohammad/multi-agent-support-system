...
@AgentRegistry.register("documentation_writer", tier="advanced", category="content")
class DocumentationWriterAgent(BaseAgent):
    """Documentation Writer."""
    
    def __init__(self):
        config = AgentConfig(name="documentation_writer", type=AgentType.COORDINATOR, model="claude-3-5-sonnet-20241022", temperature=0.5, max_tokens=4000, capabilities=[AgentCapability.KB_SEARCH, AgentCapability.DATABASE_WRITE], tier="advanced")
        super().__init__(config)
        self.logger = get_logger(__name__)
    
    async def process(self, state: AgentState) -> AgentState:
        self.logger.info("documentation_writer_started")
        ...
        try:
            ...
            content = f"**Documentation Writer**: Content generated for {topic}"
