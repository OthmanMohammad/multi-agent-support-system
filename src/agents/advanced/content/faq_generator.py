...
@AgentRegistry.register("faq_generator", tier="advanced", category="content")
class FaqGeneratorAgent(BaseAgent):
    """Faq Generator."""
    
    def __init__(self):
        config = AgentConfig(name="faq_generator", type=AgentType.COORDINATOR, model="claude-3-5-sonnet-20241022", temperature=0.5, max_tokens=4000, capabilities=[AgentCapability.KB_SEARCH, AgentCapability.DATABASE_WRITE], tier="advanced")
        super().__init__(config)
        self.logger = get_logger(__name__)
    
    async def process(self, state: AgentState) -> AgentState:
        self.logger.info("faq_generator_started")
        ...
        try:
            ...
            content = f"**Faq Generator**: Content generated for {topic}"
