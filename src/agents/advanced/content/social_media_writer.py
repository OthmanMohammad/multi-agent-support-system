...
@AgentRegistry.register("social_media_writer", tier="advanced", category="content")
class SocialMediaWriterAgent(BaseAgent):
    """Social Media Writer."""
    
    def __init__(self):
        config = AgentConfig(name="social_media_writer", type=AgentType.COORDINATOR, model="claude-3-5-sonnet-20240620", temperature=0.5, max_tokens=4000, capabilities=[AgentCapability.KB_SEARCH, AgentCapability.DATABASE_WRITE], tier="advanced")
        super().__init__(config)
        self.logger = get_logger(__name__)
    
    async def process(self, state: AgentState) -> AgentState:
        self.logger.info("social_media_writer_started")
