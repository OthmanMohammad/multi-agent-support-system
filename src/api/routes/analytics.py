...
async def get_conversation_statistics(
    ...
    Requires authentication via JWT token or API key.
    """
    logger.info("get_conversation_statistics_endpoint_called", days=days)

    result = await service.get_conversation_statistics(days=days)

    if result.is_failure:
        logger.error(
            "get_conversation_statistics_failed", days=days, error_type=type(result.error).__name__
        )
...
async def get_agent_performance(
    ...
    Requires authentication via JWT token or API key.
    """
    logger.info("get_agent_performance_endpoint_called", agent_name=agent_name, days=days)

    result = await service.get_agent_performance(agent_name=agent_name, days=days)
...
async def get_all_agents_comparison(
    ...
    Requires authentication via JWT token or API key.
    """
    logger.info("get_all_agents_comparison_endpoint_called", days=days)

    result = await service.get_agent_comparison(days=days)

    if result.is_failure:
        logger.error(
            "get_all_agents_comparison_failed", days=days, error_type=type(result.error).__name__
        )
        raise map_error_to_http(result.error)

    logger.info("get_all_agents_comparison_success", days=days, agent_count=len(result.value))
...
async def get_customer_satisfaction(
    ...
    Requires authentication via JWT token or API key.
    """
    logger.info("get_customer_satisfaction_endpoint_called", days=days)

    result = await service.get_customer_satisfaction_scores(days=days)

    if result.is_failure:
        logger.error(
            "get_customer_satisfaction_failed", days=days, error_type=type(result.error).__name__
        )
        raise map_error_to_http(result.error)
...
async def get_intent_distribution(
    ...
    Requires authentication via JWT token or API key.
    """
    logger.info("get_intent_distribution_endpoint_called", days=days)

    result = await service.get_intent_distribution(days=days)

    if result.is_failure:
        logger.error(
            "get_intent_distribution_failed", days=days, error_type=type(result.error).__name__
        )
        raise map_error_to_http(result.error)

    logger.info("get_intent_distribution_success", days=days, unique_intents=len(result.value))
...
async def get_resolution_time_trends(
    ...
    Requires authentication via JWT token or API key.
    """
    logger.info("get_resolution_time_trends_endpoint_called", days=days)

    result = await service.get_resolution_time_trends(days=days)

    if result.is_failure:
        logger.error(
            "get_resolution_time_trends_failed", days=days, error_type=type(result.error).__name__
        )
        raise map_error_to_http(result.error)
...
async def get_escalation_rate(
    ...
    Requires authentication via JWT token or API key.
    """
    logger.info("get_escalation_rate_endpoint_called", days=days)

    result = await service.get_escalation_rate(days=days)

    if result.is_failure:
        logger.error(
            "get_escalation_rate_failed", days=days, error_type=type(result.error).__name__
        )
        raise map_error_to_http(result.error)
...
async def get_kb_effectiveness(
    ...
    Requires authentication via JWT token or API key.
    """
    logger.info("get_kb_effectiveness_endpoint_called", days=days)

    result = await service.get_kb_effectiveness(days=days)

    if result.is_failure:
        logger.error(
            "get_kb_effectiveness_failed", days=days, error_type=type(result.error).__name__
        )
        raise map_error_to_http(result.error)
