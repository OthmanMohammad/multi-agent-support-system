...
class SequentialWorkflowRequest(BaseModel):
    ...
    steps: list[WorkflowStepConfig] = Field(
        ..., min_length=1, description="Workflow steps in order"
    )
...
class DebateWorkflowRequest(BaseModel):
    ...
    judge_agent: str | None = Field(None, description="Optional judge agent for final decision")
...
class DebateWorkflowResponse(BaseModel):
    ...
    judge_reasoning: str | None = Field(None, description="Judge's reasoning (if judge used)")
    execution_time: float
    error: str | None = None
...
class VerificationWorkflowRequest(BaseModel):
    ...
    verifier_agents: list[str] = Field(
        ..., min_length=1, description="Agents to verify the response"
    )
    context: dict[str, Any] | None = Field(None, description="Additional context")
    consensus_threshold: float = Field(
        default=0.7, ge=0.5, le=1.0, description="Consensus threshold (0.5-1.0)"
    )
...
class ExpertPanelWorkflowRequest(BaseModel):
    ...
    context: dict[str, Any] | None = Field(None, description="Additional context")
    moderator_agent: str | None = Field(None, description="Optional moderator agent")
    voting_method: Literal["majority", "weighted", "consensus"] = Field(
        default="majority", description="Voting method for final decision"
    )
    timeout: int = Field(default=120, ge=1, le=600, description="Overall timeout in seconds")
...
class ExpertPanelWorkflowResponse(BaseModel):
    ...
    moderator_summary: str | None = Field(None, description="Moderator's summary (if used)")
    execution_time: float
    error: str | None = None
...
class WorkflowJobResponse(BaseModel):
    ...
    workflow_type: str = Field(
        ..., description="Workflow type (sequential, parallel, debate, etc.)"
    )
    workflow_name: str
